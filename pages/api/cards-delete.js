// pages/api/cards-delete.js
// Removes a card from cards.json via the GitHub Contents API.
...
export default async function handler(req, res) {
  ...
  if (!ebayItemId) {
    return res.status(400).json({ error: 'ebayItemId is required' });
  }
  ...
  try {
    ...
    const removed = cards.find((c) => c.ebayItemId === String(ebayItemId));
    if (!removed) {
      return res.status(404).json({ error: 'No card with this eBay item ID found in cards.json' });
    }

    const updatedCards = cards.filter((c) => c.ebayItemId !== String(ebayItemId));
    const updatedContent = JSON.stringify(updatedCards, null, 2) + '\n';
